...
namespace
{
    ...
    struct MockGreedySteering
    {
        ...
        {
            const auto s = static_cast<int> (std::round (start.x));
            if (const auto g = static_cast<int> (std::round (goal.x)); storedCandidates.contains ({s, g}))
            {
...
namespace
{
    ...
    struct MockDebug
    {
        ...
        {
            double totalCost;
            std::map<std::string, double, std::less<>> softConstraints;
        };
        ...
        std::map<std::string, double, std::less<>> componentTimes;
